                           ...
                           vector<vector<Point2f> > imagePoints, float grid_width, bool release_object)
{
    ...
    if (ok) {
        ...
        std::cout << "camera intrinsc: " << cameraMatrix << endl;
...
IPhone_Camera_Info* camera_calibrate(int argc, char* argv[], char* filelist[])
{
    cout << "argc: " << argc << endl;
...
for (int i = 0; i < numFiles; i++) {
    ...
    for (int i = 0; i < mat.rows; i++) {
      for (int j = 0; j < mat.cols; j++) {
        // 데이터 타입에 따라 알맞은 변환 함수를 사용하여 값을 저장
        if (mat.type() == CV_8UC1) {
          uchar value = mat.at<uchar>(i, j);
          data[i * mat.cols + j] = static_cast<float>(value);
        } else if (mat.type() == CV_32FC1) {
          float value = mat.at<float>(i, j);
          data[i * mat.cols + j] = value;
